  ...
  return (
    <div className="lg:hidden">
      ...
      <header className="sticky top-0 z-40 flex h-16 items-center justify-between border-b border-slate-200 bg-white/90 px-4 backdrop-blur-2xl">
        ...
        <Link href={user ? "/overview" : "/"} className="flex items-center gap-2.5 min-w-0 flex-1 justify-start ml-1.5">
          ...
          <div className="flex flex-col min-w-0 justify-center text-left">
            <span className="text-[11px] sm:text-xs font-black text-slate-900 tracking-tight leading-none uppercase">
              Faith Laundry Shop
            </span>
            <span className="text-[8px] sm:text-[9px] font-extrabold uppercase tracking-widest text-brand-blue mt-0.5 leading-none">
              Management System
            </span>
          </div>
        </Link>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 overflow-hidden">
          ...
          <aside className="absolute inset-y-0 left-0 w-72 max-w-[85vw] flex flex-col bg-neutral-100 border-r border-slate-200 shadow-2xl animate-in slide-in-from-left duration-300 ease-out">
            ...
            <div className="flex h-16 items-center justify-between border-b border-slate-200 px-6 bg-white">
              <div className="flex items-center gap-2.5">
                ...
                <div className="flex flex-col min-w-0 justify-center">
                  <span className="text-[12px] font-black text-slate-900 tracking-tight leading-none uppercase">
                    Faith Laundry Shop
                  </span>
            ...
            {/* Navigation Body */}
            <nav className="flex-1 min-h-0 overflow-y-auto p-3 sm:p-4 space-y-4 sm:space-y-6 custom-scrollbar">
              {NAVIGATION_GROUPS.map((group) => {
                ...
                return (
                  <div key={group.id} className="space-y-1">
                    <h4 className="px-3 text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 mb-2">
                      {group.label}
                    </h4>
                      ...
                      {group.items.map((item) => {
                        ...
                        return (
                          <Link
                            key={item.href}
                            href={item.href}
                            className={`relative flex items-center gap-3 rounded-xl px-3 py-2.5 sm:px-4 sm:py-3 text-xs sm:text-sm font-bold transition-all uppercase tracking-wider ${isActive
                                ? "bg-brand-blue/5 text-brand-blue shadow-sm ring-1 ring-brand-blue/10"
                                : "text-slate-500 active:bg-slate-200"
                              }`}
                          >
                            <div className="relative">
                              <Icon className={`h-4.5 w-4.5 sm:h-5 sm:w-5 ${isActive ? "text-brand-blue stroke-[2.5px]" : "text-slate-400"}`} />
                            </div>
            ...
            {user && (
              <div className="mt-auto border-t border-slate-200 p-3.5 sm:p-5 bg-white space-y-3 shrink-0">
                <div className="flex items-center gap-3 px-1">
                  <div className="h-10 w-10 sm:h-11 sm:w-11 shrink-0 rounded-xl bg-gradient-to-br from-brand-blue to-brand-cyan-dark flex items-center justify-center text-xs font-black text-white shadow-md shadow-brand-blue/20 ring-1 ring-white/20">
                    {user.username.substring(0, 2).toUpperCase()}
                  </div>
                  <div className="flex flex-col min-w-0">
                    <span className="truncate text-xs sm:text-[13px] font-black text-slate-900 leading-tight">
                      {user.username}
                    </span>
                ...
                <button
                  ...
                  className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 sm:px-4 sm:py-3 text-xs font-black uppercase tracking-widest text-rose-700/80 active:bg-rose-50 transition-colors border border-rose-100/50"
                >
