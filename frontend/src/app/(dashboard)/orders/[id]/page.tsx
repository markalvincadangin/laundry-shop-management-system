            ...
            <div className="space-y-2">
              {addOns.map((a, i) => (
                <div key={i} className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 border border-slate-100 group">
                  <span className="text-sm font-bold text-slate-700">{a.name} <span className="mx-2 opacity-30">|</span> {UI_LABELS.units.PRICE_SYMBOL}{a.price.toFixed(2)}</span>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 no-print">
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-4">
            <h1 className="text-display tracking-tight text-slate-900 font-mono">
        ...
        <div className="flex flex-wrap gap-4">
          <Button variant="outline" className="h-12 px-6 gap-2 border-slate-200 text-xs font-extrabold uppercase tracking-widest" onClick={() => window.print()}>
            <Printer className="h-4 w-4" />
            {UI_LABELS.modules.orders.PRINT_RECEIPT}
          </Button>
          ...
          <Card className="overflow-hidden no-print border-slate-200 bg-white shadow-sm">
            ...
            <CardContent className="p-8">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                  <p className="text-2xl font-bold text-slate-900 tracking-tight">{order.customerName}</p>
                  <div className="flex items-center gap-2 mt-2 text-slate-500">
                    <Clock className="h-3.5 w-3.5 text-slate-400" />
                    <span className="text-xs font-bold uppercase tracking-tight">{UI_LABELS.shared.common.DATE}: {new Date(order.createdAt!).toLocaleDateString()}</span>
                  </div>
                </div>
                <div className="flex items-center gap-6">
                  <div className="text-right">
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">{UI_LABELS.shared.common.CONTACT}</p>
                    <p className="text-sm font-bold text-slate-700 font-mono">{order.contactNumber}</p>
                  </div>
                  <Link href={`/customers/${order.customerId}`}>
                    <Button variant="outline" size="sm" className="h-10 gap-2 border-slate-200 text-xs font-bold uppercase tracking-widest text-slate-600 shadow-sm">
                      {UI_LABELS.shared.common.ACTIVE}
              ...
              <div className="p-8 space-y-8">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                  <div className="space-y-2">
                    ...
                    <p className="text-lg font-bold text-slate-900">{order.weightKg} <span className="text-xs opacity-40 font-medium">{UI_LABELS.units.WEIGHT}</span></p>
                  </div>
                  <div className="space-y-2">
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                      <Package className="h-3 w-3" /> {UI_LABELS.modules.orders.LOADS}
                    </p>
                    <p className="text-lg font-bold text-slate-900">{order.totalLoads} <span className="text-xs opacity-40 font-medium">{UI_LABELS.modules.orders.LOADS}</span></p>
                  </div>
                  <div className="space-y-2">
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                      <Clock className="h-3 w-3" /> {UI_LABELS.modules.orders.EXTRA_TIME}
                    </p>
                    <p className="text-lg font-bold text-slate-900">{order.extraMinutes ?? 0} <span className="text-xs opacity-40 font-medium">{UI_LABELS.units.TIME}</span></p>
                  </div>
                  <div className="space-y-2">
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                      <Calendar className="h-3 w-3" /> {UI_LABELS.shared.common.DATE}
                    </p>
                    <p className="text-sm font-bold text-slate-900">{order.createdAt ? new Date(order.createdAt).toLocaleDateString() : "—"}</p>
                  </div>
                  <div className="space-y-2">
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                      <User className="h-3 w-3" /> {UI_LABELS.shared.common.STAFF}
                    </p>
                    <p className="text-sm font-bold text-slate-900">{order.createdByUsername || "System Agent"}</p>
                  </div>
                </div>

                <div className="space-y-4 pt-8 border-t border-slate-100">
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-slate-500 font-bold uppercase tracking-widest text-[11px]">{UI_LABELS.modules.orders.SERVICE_FEE}</span>
                    <span className="font-bold text-slate-900">{UI_LABELS.units.PRICE_SYMBOL}{order.baseAmount?.toFixed(2)}</span>
                  </div>
                  {(order.extraMinutesAmount ?? 0) > 0 && (
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-slate-500 font-bold uppercase tracking-widest text-[11px]">{UI_LABELS.modules.orders.EXTRA_TIME_FEE}</span>
                      <span className="font-bold text-slate-900">{UI_LABELS.units.PRICE_SYMBOL}{order.extraMinutesAmount?.toFixed(2)}</span>
                    </div>
                  )}
                  {(order.addonsTotalAmount ?? 0) > 0 && (
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-slate-500 font-bold uppercase tracking-widest text-[11px]">{UI_LABELS.modules.orders.EXTRAS}</span>
                      <span className="font-bold text-slate-900">{UI_LABELS.units.PRICE_SYMBOL}{order.addonsTotalAmount?.toFixed(2)}</span>
                    </div>
                  )}
                  <div className="flex justify-between items-center pt-8 mt-4 border-t-2 border-slate-900/5">
                    <span className="text-xl font-display font-black text-slate-900 uppercase tracking-tight">{UI_LABELS.shared.common.TOTAL}</span>
                    <span className="text-4xl font-display font-black text-brand-blue tracking-tight">{UI_LABELS.units.PRICE_SYMBOL}{order.grandTotal?.toFixed(2)}</span>
                  </div>
                </div>
              </div>
            )}
          ...
          {allowedNextStatuses.length > 0 && (
            <Card className="overflow-hidden border-slate-200 bg-white shadow-xl">
              <CardHeader className="bg-slate-50 border-b border-slate-100">
                <CardTitle className="text-xs font-extrabold uppercase tracking-widest text-slate-400">{UI_LABELS.modules.orders.NEXT_STEP}</CardTitle>
              </CardHeader>
                    ...
                    return (
                      <Button
                        ...
                        className={nextStatus === ORDER_STATUS.CANCELLED 
                          ? "text-rose-600 border-rose-200 hover:bg-rose-50 font-bold uppercase text-xs tracking-widest h-14 shadow-sm" 
                          : "bg-brand-blue shadow-lg shadow-brand-blue/20 font-extrabold uppercase text-xs tracking-widest h-16 active:scale-95 transition-all"}
                      >
                        {label}
